...
#把JSON数据和可选的二进制数据打包成符合 STEP 协议的 TCP 数据包（说白了 这就是打包成规定格式）
...
#从 TCP 连接中接收一个完整的数据包（packet），并拆分出 JSON 数据和二进制数据。
...
#type = DATA 时，服务器就会进入这个 data_process() 来执行
...
#file_process() 专门处理大文件上传 / 下载，支持分块传输
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_GET:
        ...
        logger.info(f'<-- Plan: file size {file_size}, total block number {total_block}, block size {block_size}.')
        connection_socket.send(
    ...
    if request_operation == OP_DOWNLOAD:
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is False:
            if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is True:
                ...
                connection_socket.send(
                    make_response_packet(OP_DOWNLOAD, 404, TYPE_FILE,
                                         f'The "key" {json_data[FIELD_KEY]} is not completely uploaded. '
            ...
            logger.error(f'<-- The "key" {json_data[FIELD_KEY]} is not existing.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 404, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is not existing.', {}))
            return

        if FIELD_BLOCK_INDEX not in json_data.keys():
            logger.error(f'<-- The "block_index" is compulsory.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" is compulsory.', {}))
            return
        file_path = join('file', username, json_data[FIELD_KEY])
        file_size = getsize(file_path)
        block_size = MAX_PACKET_SIZE
...
#服务器就是在这里解析、验证、分发的
#验证 token 合法性
""" 1.  它循环调用 get_tcp_packet() 收包。
	2.	检查 JSON 是否包含必要字段。
	3.	如果是 LOGIN 请求 → 验证用户名密码并生成 token。
	4.	否则 → 验证 token 合法性。
	5.	根据 type 分别调用：
	•	data_process()（普通数据）
	•	file_process()（文件操作）
...
def tcp_listener(server_ip, server_port):
    ...
    :param server_port
    :return: None
    每接入一个客户端，就启动一个线程运行 STEP_service() 来处理该连接
    """
    global logger                                                #用它输出服务器运行状态、连接信息或错误提示
    server_socket = socket(AF_INET, SOCK_STREAM)
    server_socket.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
    server_socket.bind((server_ip, int(server_port)))            #绑定 IP 和端口
    server_socket.listen(20)                                     #参数 20 是“最大等待队列长度”
    logger.info('Server is ready!')
    logger.info(
        f'Start the TCP service, listing {server_port} on IP {"All available" if server_ip == "" else server_ip}')
    while True:                                                  #不断接受来自客户端的消息
        try:
...
def main():
    ...
    os.makedirs('data', exist_ok=True)#在文件系统中创建目录
    os.makedirs('file', exist_ok=True)
    os.makedirs('tmp', exist_ok=True)

    tcp_listener(server_ip, server_port)#解析命令行参数 _argparse() 获取 server_ip 和 server_port
